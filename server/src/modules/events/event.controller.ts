...
export async function create(req: Request, res: Response): Promise<void> {
  const event = await eventService.createEvent(req.userId as string, req.valid.body as CreateEventInput);
  sendData(res, event, 201);
}

export async function update(req: Request, res: Response): Promise<void> {
  const { id } = req.valid.params as { id: string };
  const event = await eventService.updateEvent(req.userId as string, id, req.valid.body as UpdateEventInput);
  sendData(res, event);
}
...
export async function mine(req: Request, res: Response): Promise<void> {
  sendData(res, await eventService.getMyEvents(req.userId as string));
}
